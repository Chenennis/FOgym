...
class ModelBasedPipeline:
    ...
    def _load_device_config(self):
        ...
        try:
            ...
            logger.warning(f"Failed to load device config: {e}, using default device config")
    ...
    def _convert_device_config(self, device_config: Dict) -> Dict:
        """Convert device config to device model params"""
        device_params = {}
        device_type = str(device_config['device_type'])
    ...
    def generate_flexoffers(self, timestep: int) -> Dict[str, Dict[str, Any]]:
        ...
        logger.info(f"Generated {total_fo_count} FlexOffers for timestep {timestep}")
    ...
    def aggregate_flexoffers(self, fo_systems, timestep):
        ...
        logger.info(f"Time step {timestep} aggregation completed: total {total_aggregated_fos} aggregated FOs")
        return aggregated_results 

    def _aggregate_lp(self, flexoffers, manager_id):
        """Aggregate FlexOffer using Longest Profile"""
        if not flexoffers:
            return []
        ...
        while remaining_fos:
            ...
            logger.info(f"Aggregated FO: {aggregated_fo.device_id}, contains {len(selected_fos)} FOs, total energy: {current_energy:.2f}kWh")
    ...
    def _aggregate_dp(self, flexoffers, manager_id):
        """Aggregate FlexOffer using Dynamic Profile"""
        if not flexoffers:
            return []
    ...
    def trade_flexoffers(self, aggregated_results, timestep):
        ...
        logger.info(f"Time step {timestep} trading completed: total {len(traded_fo_ids)} traded FOs")
    ...
    def run(self, num_timesteps=1):
        """Run Pipeline"""
        logger.info(f"Running ModelBased Pipeline, experiment ID: {self.experiment_id}, total timesteps: {num_timesteps}")
...
def run_pipeline(config_path=None, num_timesteps=24, aggregation_method=None, trading_method=None, disaggregation_method=None, save_results=True, seed=None):
    """Run ModelBased Pipeline
    
    Parameters:
        config_path: Configuration file path
        num_timesteps: Total timesteps
        aggregation_method: Aggregation method ("LP" or "DP")
        trading_method: Trading method ("bidding" or "market-clearing")
        disaggregation_method: Disaggregation method ("proportional" or "average")
...
if __name__ == "__main__":
    ...
    print(f"- Manager number: {config.num_managers}")
    print(f"- Users per manager: {config.users_per_manager} (total {sum(config.users_per_manager)} users)")
