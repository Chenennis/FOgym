...
Pmin = -5;
Pmax = 5;
PminP = Pmin*percentage;
PmaxP = Pmax*percentage;
T = 6;
%decay = 0.99;
decay = 1;
Cost = 0;
...
% load('c:\Users\siksn\Downloads\M.mat');
% load('c:\Users\siksn\Downloads\N.mat');
...
elseif Type == 0.4
if Q(1) >= T/2*PmaxP*(1 - eta) + Qmax - PmaxP
        maxsum = T/2*PmaxP*(1 - eta^2) + eta* (Qmax-Q(1));
    elseif Q(1) >= (T/2*(1 - eta) - eta)*PmaxP + Qmax - PmaxP
        maxsum = (T/2-1)*(1/eta - eta)*PmaxP + (Qmax-Q(1))/eta;
    elseif Q(1) >= (T/2*(1 - eta) - 2*eta - 1)*PmaxP + Qmax
        maxsum = (T/2+1)*PmaxP*(1 - eta^2) + eta* (Qmax-Q(1));
    else
        maxsum = (T/2-2)*(1/eta - eta)*PmaxP + (Qmax-Q(1))/eta;
end
    for k = 1:T
        Q(k+1) = decay*Q(k) + eta *max(0,psum(k+1)) + 1/eta * min(0,psum(k+1));
%   VCost = VCost + Cost*up((TT-1)*U+k);
...
%end
%constraints = [constraints, min(0,VPmin) <= un <= min(VPmax,0), max(0,VPmin) <= up <= max(0,VPmax), VQmin <= Q <= VQmax];%, norm(un.*up) <= 0.01];
 constraints = [constraints, Qflexmin <= sum(psum(2:T+1)) <= maxsum, repelem(eta*PminP,T+1) <= psum <= repelem(PmaxP,T+1)];% VQmin <= Q <= VQmax];%, un <= 0, 0 <= up,  norm(un.*up) <= 0.01]
net1 = net + psum(2:T+1);
...
%constraints = [constraints, min(0,VPmin) <= un <= min(VPmax,0), max(0,VPmin) <= up <= max(0,VPmax), VQmin <= Q <= VQmax];%, norm(un.*up) <= 0.01];
% constraints = [constraints, VQmin <= Q <= VQmax];%, un <= 0, 0 <= up,  norm(un.*up) <= 0.01]
    constraints = [constraints, repelem(eta*PminP,T+1) <= psum <= repelem(PmaxP,T+1)];
net1 = net + psum(2:T+1);
    else
...
%end
RMatrix(14,counter) = totalImbalanceCost+RMatrix(1,counter);
QStatus(counter+1) = max(min(Q1(T+1),Qmax),0);

%filename = sprintf('/home/ubuntu/imbalances_Q0is0_Tis6_Typeis2_08_daysare1to372part%d.xlsx',parallel);
%    filename = sprintf('C:/Users/Fab/Documents/Postdoc/A/correcttesting3_Q0is0_Tis6_Typeis0_1_daysare1to372part%dL%d.xlsx',parallel,counter);
%    writematrix(RMatrix,filename)
    end
    
    filename = sprintf('/home/ubuntu/imbalances_Q0is0_Tis6_Typeis%d__percentageis%d_daysare1to372part%d.xlsx',Type,percentage,parallel);
