...
days = ceil(119/par);%number of days for iteration
...
for counter = 1:Length
    ...
    spotDFO = M1(1+sposDFO+daysahead:T0+sposDFO+daysahead,1:1)'; %spot prices
    imbalanceDFO = M1(1+sposDFO+daysahead:T0+sposDFO+daysahead,1:1)'; %imbalance prices
...
%     if mod(counter,7) > 4
%         evv = value(UFO.getEnergyVars);
%         evars = EUnc * evv;
%         UFOcost = spotprices(1:T) * transpose(evars);
%     end
    ...
    if cloud == 1
        filename = sprintf("/home/ubuntu/EVsingleHighPenalties%d_%d_%d_%dT_%d.xlsx",EE,NumBatteries,parallel,pt1,testcase);
    else
