# Set OpenMP environment variables to avoid multiple initialization conflicts
import os
...
# Add MAPPO onpolicy module path
...
# Import logging configuration control
try:
...
try:
    ...
    logger.warning("Global observation space module not available, will use default module observation")

# Custom RL algorithm registry
class RLRegistry:
    """RL algorithm registry for registering and retrieving custom RL algorithms"""
    ...
    def register(cls, name: str, agent_class):
        ...
        # Determine if the current process should output logs
        ...
        # Only output logs in main process and on first registration
        if (not cls._initialized and current_process == main_process and 
            name not in cls._registered_algorithms):
            logger.info(f"Registered RL algorithm: {name}")
...
# Try to import FOMAPPO algorithm
try:
    ...
    logger.warning("FOMAPPO algorithm not available, please check algorithms/MAPPO/fomappo directory")

# Try to import FOMAPPO shared policy adapter
try:
    ...
    logger.warning("FOMAPPO algorithm (shared policy) not available, please check algorithms/MAPPO/fomappo directory")

# Try to import FOMAIPPO algorithm
try:
    ...
    logger.warning("FOMAIPPO algorithm (independent policy) not available, please check algorithms/MAPPO/fomappo directory")

# Try to import FOMADDPG algorithm
try:
    ...
    logger.warning("FOMADDPG algorithm not available, please check algorithms/MADDPG/fomaddpg directory")

# Try to import FOMATD3 algorithm
try:
    ...
    logger.warning("FOMATD3 algorithm not available, please check algorithms/MATD3/fomatd3 directory")

# Try to import FOSQDDPG algorithm
try:
    ...
    logger.warning("FOSQDDPG algorithm not available, please check algorithms/SQDDPG/fosqddpg directory")

# Try to import FOModelBased algorithm (deprecated)
try:
    import sys
    import os
    ...
    logger.warning(f"FOModelBased algorithm not available: {e}, please check algorithms/Model-based/fomodelbased directory")

# Register default algorithms - Note: Multi-agent algorithms should not be registered with RLRegistry
# RLRegistry is designed for single-agent algorithms, multi-agent algorithms have a special initialization process
if FOMAPPO_available and FOMAPPO is not None:
    RLRegistry.register("fomappo", FOMAPPO)
# Remove FOMADDPG registration - it's a multi-agent algorithm that should be initialized through the adapter during training
...
class FOPipeline:
    """FlexOffer complete process management class"""
    ...
    def __init__(self, config: Dict):
        ...
        """
        self.config = config
        self.time_horizon = config.get("time_horizon", 24)  # Time range for each episode (hours)
        self.time_step = config.get("time_step", 1.0)  # Length of each time step (hours)
        
        self.num_episodes = config.get("num_episodes", 100)  # Number of training episodes, each episode = 24 hours
        if not isinstance(self.num_episodes, int) or self.num_episodes <= 0:
            logger.warning(f"Invalid num_episodes value: {self.num_episodes}, setting to default value 1")
            self.num_episodes = 1
        elif self.num_episodes > 100:
            logger.warning(f"num_episodes value too large: {self.num_episodes}, may result in excessive training time")
        
        logger.info(f"Training configuration: num_episodes={self.num_episodes}")
        
        # Validate time configuration
        if self.time_step != 1.0:
            logger.warning(f"Time step length set to {self.time_step} hours, recommended to use 1.0 hour")
        
        # Calculate number of time steps per episode (should be 24 steps, from 0 to 23)
        self.steps_per_episode = int(self.time_horizon / self.time_step)
        if self.steps_per_episode != 24:
            logger.warning(f"Each episode has {self.steps_per_episode} time steps, recommend 24 time steps (0-23)")
        ...
        # Default to actual multi-agent environment configuration
        ...
        self.devices_per_user = config.get("devices_per_user", {
            DeviceType.BATTERY: (0, 1),    
            DeviceType.HEAT_PUMP: (1, 1),  
            DeviceType.EV: (0, 1),         
            DeviceType.PV: (0, 1),          
            DeviceType.DISHWASHER: (1, 1)   
        })
        
        # Algorithm selection
        self.rl_algorithm = config.get("rl_algorithm", "fomappo")
        self.actual_running_algorithm = self.rl_algorithm  
        
        # Define built-in multi-agent algorithm list
        builtin_multi_agent_algorithms = [
            "fomappo",      # FlexOffer multi-agent algorithm based on MAPPO
            "fomaippo",     # FlexOffer multi-agent algorithm based on MAPPO
        ...
        # Initialize rl_agents only for algorithms that support single user agents
        if self.rl_algorithm == "fomappo":
            self.rl_agents[self.rl_algorithm] = {}
        elif self.rl_algorithm == "fomodelbased":
            # FOModelBased is not a multi-agent algorithm
        ...
        self.training_loss_history = {}  # For recording loss function values for each episode
        
        # Generate unique experiment identifier (delayed until actual algorithm is determined)
        ...
        logger.info(f"FOPipeline initialization complete, mode: RL={self.rl_algorithm}, "
    ...
    def _generate_experiment_id(self):
        """Generate unique experiment identifier (using the actual running algorithm)"""
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        
        # Build configuration string (using the actual running algorithm)
    ...
    def _update_actual_algorithm(self, algorithm_name):
        """Update actual running algorithm name and generate experiment ID"""
        ...
        # Initialize algorithm-specific parts of the training history
    ...
    def _save_training_history_with_backup(self, prefix=""):
        """Enhanced training history saving method with multiple backups"""
        ...
        if self.experiment_id is None:
            ...
            logger.warning(f"experiment_id was None during backup, generated: {self.experiment_id}")
        
        # Method 1: CSV save (primary method)
        try:
        ...
        try:
            ...
            logger.info(f"✅ JSON backup saved successfully: {json_file}")
        except Exception as e:
            logger.error(f"JSON backup save failed: {e}")
        
        # Method 3: Plain text backup
        try:
            ...
            logger.info(f"✅ Text backup saved successfully: {txt_file}")
        except Exception as e:
            logger.error(f"Text backup save failed: {e}")
    
    def _force_save_training_history(self, training_data, algorithm_name):
        """Force save training history data - last resort measure"""
        if not training_data:
            logger.warning("No data to force save")
            return
        
        # Add debug information
        logger.info(f"Force saving training history data, type: {type(training_data)}")
        ...
        try:
            ...
            with open(filepath, 'w', encoding='utf-8') as f:
                f.write(f"Force saved training history - {algorithm_name}\n")
                f.write(f"Time: {datetime.now()}\n")
                f.write(f"Experiment ID: {self.experiment_id}\n\n")
                
                # Handle complex nested dictionary situations
                if isinstance(training_data, dict) and 'manager_rewards' in training_data:
                    ...
                    for manager_id, rewards in training_data['manager_rewards'].items():
                        ...
                        if not isinstance(rewards, (list, np.ndarray)):
                            f.write(f"  Warning: Rewards is not a list or array, but {type(rewards)}\n")
                            continue
                            ...
                            try:
                                ...
                                for r in rewards:
                                    if isinstance(r, (int, float, np.number)):
                                        ...
                                        logger.warning(f"Skipping non-numeric reward: {r} type: {type(r)}")
                    ...
                    if 'timestep_rewards' in training_data:
                        f.write("\nTimestep reward components:\n")
                        for i, tr in enumerate(training_data['timestep_rewards']):
                            f.write(f"Timestep {i+1}: trade_value={tr.get('trade_value', 0):.2f}, " +
                        ...
                        for i, reward in enumerate(rewards):
                            ...
                            if rewards:
                                try:
                                    numeric_rewards = []
                                    for r in rewards:
                                        # Check if it's a dictionary type (nested training record)
                                        if isinstance(r, dict) and 'episode_reward' in r:
                                            ...
                                            logger.info(f"Extracting reward value from dictionary: {r['episode_reward']}")
            ...
            logger.info(f"✅ Force save successful: {filepath}")
            return filepath
        except Exception as e:
    ...
    def _save_training_history_to_csv(self, algorithm_name):
        ...
        if algorithm_name.upper() == "FOMODELBASED" and hasattr(self, 'fomodelbased_results'):
            try:
                # Directly generate CSV using fomodelbased_results
                import pandas as pd
                ...
                for manager_id, manager_rewards in self.fomodelbased_results.items():
                    # Use single episode with multiple timesteps
        ...
        if not hasattr(self, 'training_history') or not self.training_history:
            logger.warning("Training history does not exist or is empty, initializing default training history")
            self._init_default_training_history()
            
        # 2. Check if episode_rewards exists
        if not self.training_history.get("episode_rewards"):
            logger.warning("No episode_rewards in training history, creating default training history")
            self._init_default_training_history()
        ...
        if isinstance(self.training_history["episode_rewards"], dict):
            ...
            if not has_data:
                logger.warning("All Manager data in training history dictionary is empty, creating default training history")
                self._init_default_training_history()
        elif isinstance(self.training_history["episode_rewards"], list):
            if len(self.training_history["episode_rewards"]) == 0:
                logger.warning("Training history list is empty, creating default training history")
                self._init_default_training_history()
        ...
        if isinstance(self.training_history["episode_rewards"], dict):
            has_data = any(len(rewards) > 0 for rewards in self.training_history["episode_rewards"].values())
            if not has_data:
                logger.warning("All Manager data in the training history dictionary is empty")
                return
        elif isinstance(self.training_history["episode_rewards"], list):
            if len(self.training_history["episode_rewards"]) == 0:
        ...
        if not algorithm_name:
            ...
            logger.warning(f"algorithm_name is empty, using: {algorithm_name}")
        
        if self.experiment_id is None:
            timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
            self.experiment_id = f"training_{timestamp}"
            logger.warning(f"experiment_id is None when saving, generating: {self.experiment_id}")
        
        os.makedirs(self.results_dir, exist_ok=True)
        
        logger.info(f"🔍 Starting to save training history, algorithm: {algorithm_name}")
            ...
            if isinstance(self.training_history["episode_rewards"], dict):
                ...
                for manager_id, rewards in self.training_history["episode_rewards"].items():
                    for episode, reward in enumerate(rewards):
                        # Get actual training loss information from training_loss_history
                        policy_loss = 0.0
                        value_loss = 0.0
                        entropy = 0.0
                        ...
                        reward_value = None
                        try:
                            # Case 1: Reward is a dictionary type containing an 'episode_reward' key
                            if isinstance(reward, dict) and 'episode_reward' in reward:
                                reward_value = float(reward['episode_reward'])
                                logger.debug(f"Extracting reward value from dictionary: {reward_value}")
                            # Case 2: Reward is a dictionary type but doesn't contain an 'episode_reward' key
                                ...
                                if numeric_keys:
                                    ...
                                    logger.debug(f"Extracting alternative reward key '{numeric_keys[0]}' from dictionary: {reward_value}")
                                else:
                                    reward_value = float(len(reward)) * 0.1
                                    logger.warning(f"Unable to extract numeric reward from dictionary, using fallback value: {reward_value}")
                            # Case 3: Reward is a numeric type
                            elif isinstance(reward, (int, float, np.number)):
                                reward_value = float(reward)
                            # Case 4: Reward is another type
                            else:
                                ...
                                try:
                                    ...
                                    reward_value = 0.1
                                    logger.warning(f"Unable to convert reward to numeric value, type: {type(reward)}, using default value: {reward_value}")
                        except Exception as e:
                            reward_value = 0.1
                            logger.error(f"Error processing reward: {e}, using default value: {reward_value}")
                        
                        # Ensure reward_value is not None
                        if reward_value is None:
                            reward_value = 0.1
                            logger.warning(f"Reward value is None, using default value: {reward_value}")
                        ...
                        try:
                            ...
                            # Calculate the average of the most recent 10 rewards
                        ...
                        # Use the extracted reward value
                        history_rows.append({
                ...
                for episode in range(total_episodes):
                        # Calculate the total reward for all managers in the current episode
                        try:
                            episode_rewards = []
                            ...
                            # Calculate the cumulative reward for all managers
                            ...
                            # Calculate the average total reward for the most recent 10 episodes
                            recent_totals = []
                        ...
                        # Calculate the average loss value for all managers
                        avg_policy_loss = 0.0
                        avg_value_loss = 0.0
                        avg_entropy = 0.0
                    ...
                    try:
                        if isinstance(reward, dict) and 'episode_reward' in reward:
                            ...
                            try:
                                ...
                                reward_value = 0.1
                                logger.warning(f"Unable to convert reward to a numeric value, type: {type(reward)}, using default value: {reward_value}")
                    except Exception as e:
                        reward_value = 0.1
                        logger.error(f"Error processing reward: {e}, using default value: {reward_value}")
                    
                    # Calculate cumulative rewards and average rewards
                    try:
                        previous_rewards = []
                        ...
                        logger.error(f"Error calculating cumulative rewards: {e}")
                        cumulative_reward = episode * 0.1
                        avg_reward_last_10 = 0.1
            ...
            if history_rows:
                ...
                try:
                    ...
                    if os.path.exists(csv_file) and os.path.getsize(csv_file) > 0:
                        logger.info(f"✅ {algorithm_name} training history has been saved to {csv_file}, totaling {len(history_rows)} rows")
                        print(f"✅ Training history has been saved to {os.path.basename(csv_file)}, total {len(history_rows)} rows")
                        save_success = True
                ...
                # 🔧 Method 2: If pandas fails, use the standard csv module
                if not save_success:
                    try:
                        import csv
                        ...
                        if os.path.exists(csv_file) and os.path.getsize(csv_file) > 0:
                            logger.info(f"✅ {algorithm_name} training history has been saved to {csv_file} using csv module")
                            save_success = True
                        else:
                ...
                # 🔧 Method 3: If all fail, create a basic text backup
                if not save_success:
                    try:
                        ...
                        with open(backup_file, 'w', encoding='utf-8') as f:
                            ...
                            for row in history_rows:
                                f.write(f"{row}\n")
                        logger.info(f"🔧 Emergency text backup has been saved to {backup_file}")
                    except Exception as e:
    ...
    def _record_training_loss(self, manager_id: str, episode: int, policy_loss: float, value_loss: float, entropy: float = 0.0):
        ...
        try:
            ...
            logger.error(f"Error recording loss value: {e}")
    ...
    def _generate_csv_filename(self, data_type: str, algorithm_name: Optional[str] = None) -> str:
        ...
        Args:
            data_type: Data type, such as 'rewards', 'pipeline_results'
    ...
    def _init_global_observation_manager(self):
        ...
        try:
            ...
            if self.global_observation_config_file and os.path.exists(self.global_observation_config_file):
                ...
                logger.info(f"Loaded global observation configuration from {self.global_observation_config_file}")
            else:
                self.global_observation_manager = GlobalObservationManager()
                logger.info("Using default global observation configuration")
        except Exception as e:
    ...
    def _setup_components(self):
        ...
        # Register managers to trading pool after they are created
    ...
    def _setup_device_models(self):
        ...
        # Initialize price loader - load prices from grid_price.csv first
        from fo_generate.price_loader import PriceLoader
        self.price_loader = PriceLoader("data")
        
        # Generate price data for the current time range
        ...
        try:
            ...
            logger.warning(f"Price loader failed: {e}, using alternative")
            ...
            if price_data_file and os.path.exists(price_data_file):
                ...
                prices = np.random.uniform(0.1, 0.3, self.time_horizon)  # Simulate price data
                self.price_data = pd.DataFrame({"timestamp": timestamps, "price": prices})
                logger.info("Using generated test price data")
        ...
        if weather_data_file and os.path.exists(weather_data_file):
            ...
            temperatures = np.random.uniform(15, 25, self.time_horizon)  
            irradiances = np.random.uniform(200, 800, self.time_horizon)  
    ...
    def _setup_rl_agents(self):
        """Set up RL agents"""
        if not self.envs:
            logger.warning("Please call _create_environments first")
            return
        ...
        if self.rl_algorithm in ["fomappo", "fomaippo", "fomaddpg", "fomatd3", "fosqddpg"]:
            logger.info(f"{self.rl_algorithm.upper()} is a multi-agent algorithm, initializing multi-agent environment in training phase")
            ...
            if agent_class is not None:
                ...
                for user_id, env in self.envs.items():
                    ...
                    try:
                        ...
                        # Fallback to FOMAPPO
                        logger.info("Falling back to FOMAPPO algorithm")
    ...
    def _setup_managers_and_users(self):
        ...
        # Determine the number of users each manager manages
        if self.num_managers == 4 and self.num_users == 36:
            self.users_distribution = [6, 10, 8, 12]
            logger.info(f"Using custom user distribution: {self.users_distribution}")
        else:
            ...
            logger.info(f"Using average user distribution: {self.users_distribution}")
        
        # Verify total number of users
        assert sum(self.users_distribution) == self.num_users, f"User distribution total {sum(self.users_distribution)} does not equal total number of users {self.num_users}"
        ...
        for m in range(self.num_managers):
            ...
            coverage_area = random.uniform(1, 5)  # Random coverage area (square kilometers)
            ...
            for u in range(start_user_idx, end_user_idx):
                ...
                for device_type, (min_count, max_count) in self.devices_per_user.items():
                    ...
                    for d in range(count):
                        ...
                        if device_type == DeviceType.BATTERY:
                            ...
                            now = datetime.now()
                            arrival_time = datetime(now.year, now.month, now.day, 18, 0)  # 6pm arrival
                            departure_time = datetime(now.year, now.month, now.day + 1, 7, 30)  # 7:30am departure
                            ...
                            params = PVParameters(
                                ...
                                area=capacity * 5,  # Assume 5 square meters per kW
                                location="roof",
                                tilt_angle=30.0,
                                azimuth_angle=180.0,
                                weather_dependent=True,
                            ...
                            now = datetime.now()
                            deployment_time = now  # User starts at deployment time
                ...
                if len(user.devices) == 0:
                    logger.warning(f"User {user.user_id} has no devices, adding default battery device")
    ...
    def _register_managers_to_trading_pool(self):
        """Register managers to the trading pool"""
    ...
    def _setup_scheduler(self):
        ...
        # 🔧 Important fix: correctly calculate time_steps_per_hour
        # Original problem: when time_step > 1, int(1/time_step) becomes 0, causing total_steps=0
        # Fix: ensure time_steps_per_hour is at least 1, and logic is correct
        if self.time_step <= 1.0:
            # Standard case: time_step=1.0 hour, time_steps_per_hour=1
            time_steps_per_hour = int(1 / self.time_step)
        else:
            # Special case: time_step>1.0 hour, time steps per hour should be a fraction
            # However, due to system design, we use 1 as the minimum value and adjust time_horizon
            time_steps_per_hour = 1
            logger.warning(f"time_step={self.time_step}>1.0, adjusting time_steps_per_hour=1")
        
        # Ensure time_steps_per_hour is at least 1 (to avoid dimension issues caused by 0 values)
        ...
        # Schedule manager - ensure managers exist
        if hasattr(self, 'managers') and self.managers:
            ...
            logger.info(f"Initialize schedule manager, number of managers: {len(self.managers)}, time range: {self.time_horizon} hours, disaggregation algorithm: {self.disaggregation_method}")
        else:
            self.schedule_manager = ScheduleManager(
                managers=[],
                trading_pool=self.trading_pool,
            ...
            logger.warning("No managers, using empty schedule manager")
    ...
    def _create_environments(self):
        """Create RL environments for each user"""
        if not self.managers or not self.users:
            logger.warning("User or manager not initialized, cannot create environments")
            return
        ...
        for user in self.users:
            if not user.devices:
                logger.warning(f"User {user.user_id} has no devices, skipping")
                continue
            ...
            if not hasattr(env.action_space, 'shape') or env.action_space.shape is None or len(env.action_space.shape) == 0 or env.action_space.shape[0] == 0:
                logger.warning(f"User {user.user_id} environment action space is invalid, skipping")
                continue
    ...
    def train_rl_agents(self):
        ...
        print(f"🔧 Time steps/episode: {self.steps_per_episode}")
        print("=" * 50)
        
        logger.info(f"🚀 Starting RL agent training, algorithm: {self.rl_algorithm}, training episodes: {self.num_episodes}")
        ...
        # Train agents based on the selected algorithm
        try:
            if self.rl_algorithm == "fomappo":
                # 🔧 Using FOMAPPO (shared policy architecture)
                print("Selected algorithm: FOMAPPO ")
                logger.info("Using FOMAPPO")
                self._train_fomappo_agents()
            elif self.rl_algorithm == "fomaippo":
                # 🔧 Using FOMAIPPO (independent policy architecture)
                print("Selected algorithm: FOMAIPPO ")
                logger.info("Using FOMAIPPO")
                self._train_fomaippo_agents()
            elif self.rl_algorithm == "fomaddpg":
                print("Selected algorithm: FOMADDPG")
                self._train_fomaddpg_agents_optimized()
            elif self.rl_algorithm == "fomatd3":
                print("Selected algorithm: FOMATD3")
                self._train_fomatd3_agents_with_adapter()
            elif self.rl_algorithm == "fosqddpg":
                print("Selected algorithm: FOSQDDPG")
                self._train_fosqddpg_agents_with_adapter()
            elif self.rl_algorithm == "fomodelbased":
                print("Selected algorithm: FOModelBased")
                print("📊 Starting FOModelBased traditional optimization evaluation...")
                logger.info("📊 Starting FOModelBased traditional optimization evaluation...")
                self.run_fomodelbased_evaluation()
                print("✅ FOModelBased evaluation complete!")
            elif self.custom_rl_algorithm and self.rl_algorithm in self.rl_agents:
                print(f"🤖 Selected algorithm: {self.rl_algorithm} (custom)")
            # Try to call custom algorithm training method
                self._train_custom_agents()
            else:
                print(f"⚠️ Unsupported RL algorithm: {self.rl_algorithm}, falling back to FOMAIPPO")
                logger.warning(f"Unsupported RL algorithm: {self.rl_algorithm}, will use FOMAIPPO")
                self._train_fomaippo_agents()
    ...
    def _train_fomappo_agents(self):
        ...
        print("\n🔧 Entering FOMAPPO training method...")
        logger.info("🔧 Starting _train_fomappo_agents method")
        ...
        try:
            ...
            logger.info("✅ Successfully imported train_fomappo_shared_policy, calling fixed training method")
            result = train_fomappo_shared_policy(self)
            print("✅ External training method execution completed")
            
            # Process object returned by external training method
            if isinstance(result, dict) and result.get('status') == 'success':
                logger.info("✅ External training method completed successfully, setting adapter references")
                ...
                # Ensure training history is properly set
                ...
                logger.info(f"Verification: hasattr(self, 'fomappo_adapter') = {hasattr(self, 'fomappo_adapter')}")
                ...
                # Even if training fails, ensure environment and adapter are saved
            ...
            print("🔄 Fall back to integrated training method...")
            logger.info("🔄 Fall back to integrated training method")
            result = self._train_fomappo_agents_integrated()
            print("✅ Integrated training method executed")
            return result
        except Exception as e:
    ...
    def _train_fomappo_agents_original(self):
        ...
        try:
            ...
            # Add experience buffer and batch update mechanism
    ...
    def _get_pipeline_observations(self) -> Dict[str, np.ndarray]:
        ...
        # Use existing multi-agent environment to get observations
    ...
    def _get_manager_state(self, manager) -> np.ndarray:
        ...
        # Extend to fixed dimension (e.g., 40 private information)
    ...
    def _get_environment_state(self) -> np.ndarray:
        ...
        # Extend to 18 public information
    ...
    def _get_limited_others_state(self, current_manager_id: str) -> np.ndarray:
        """Get limited information of other Managers"""
        others_features = []
        ...
        # Extend to 15 other information
    ...
    def _get_manager_action_dim(self) -> int:
        """Get Manager action space dimension"""
        ...
        return max(max_devices, 10)  # At least 10 action dimensions
    ...
    def _generate_flexoffers_with_actions(self, actions: Dict[str, np.ndarray], timestep: int):
        ...
        if self.rl_algorithm == "fomappo" and hasattr(self, 'multi_agent_env'):
            try:
                # Directly use multi-agent environment to generate FlexOffer
                logger.info(f"Using {self.rl_algorithm} multi-agent environment to generate FlexOffer...")
        ...
        for manager_id, action in actions.items():
            if manager_id in fo_systems or any(manager_id in str(key) for key in fo_systems.keys()):
                # Actions can affect:
                # - Energy range adjustment (action[0:5])
                # - Time window flexibility (action[5:10])  
                # - Priority weight (action[10:15])
    ...
    def _calculate_pipeline_rewards_from_results(self, pipeline_results: Dict, manager_ids: List[str]) -> Dict[str, float]:
        ...
        for manager_id in manager_ids:
            ...
            # Trade reward: successful trade count
    ...
    def _train_simple_pipeline_training(self):
        """Simplified Pipeline training (fallback)"""
        logger.warning("Using simplified Pipeline training as fallback")
        
        manager_ids = [manager.manager_id for manager in self.managers]
        total_rewards = {manager_id: [] for manager_id in manager_ids}
        
        for episode in range(min(self.num_episodes, 10)):
            logger.info(f"Simplified training Episode {episode+1}/10")
    ...
    def _train_fomaddpg_agents(self):
        """Train FOMADDPG multi-agent algorithm - using FOMADDPG adapter"""
        ...
        try:
            ...
            logger.info("✅ FOMADDPG adapter initialization completed")
                ...
                if (episode + 1) % 10 == 0:
                    ...
                    try:
                        ...
                        if isinstance(training_stats, dict):
                            ...
                            logger.info(f"  🚀 Training iterations: {iterations}, experience buffer size: {buffer_size}")
            ...
            # Training completed processing
            logger.info("🎉 FOMADDPG adapter training completed!")
            
            # Save training history
            try:
                # Use actual recorded episode rewards
            ...
            # Training history saving - using multiple saving methods to ensure data is not lost
    ...
    def _train_fomaddpg_agents_original(self):
        ...
        try:
            ...
            if manager_ids:
                ...
                logger.error("Cannot get state and action space dimensions")
                return
            ...
            if FOMADDPG_available and FOMADDPG is not None:
                ...
                logger.info("Original FOMADDPG algorithm initialization successful")
    ...
    def _train_fomatd3_agents(self):
        ...
        try:
            ...
            if manager_ids:
                # Calculate global state dimension (flattened multi-agent observation)
                single_agent_obs_dim = len(sample_obs[manager_ids[0]])
                state_dim = single_agent_obs_dim * num_managers  # Global state = single agent observation × agent count
                action_dim = multi_env.action_spaces[manager_ids[0]].shape[0]
            ...
            if FOMATD3_available and FOMATD3 is not None:
                ...
                logger.info("FOMATD3 algorithm initialization successful")
                
                # Training loop
                total_rewards = []
                
                for episode in range(self.num_episodes):
                    logger.info(f"\n========== Starting Episode {episode+1}/{self.num_episodes} (FOMATD3) ==========")
                    ...
                    # Run 24 time steps for each episode
                    for timestep in range(self.steps_per_episode):
                        logger.info(f"Episode {episode+1}, time step {timestep} (hour {timestep})")
    ...
    def _train_fosqddpg_agents(self):
        ...
        try:
            ...
            if FOSQDDPG_available and FOSQDDPG is not None:
                ...
                logger.info("FOSQDDPG algorithm initialization successful")
                
                # Training loop
                total_rewards = []
                
                for episode in range(self.num_episodes):
                    logger.info(f"\n========== Starting Episode {episode+1}/{self.num_episodes} (FOSQDDPG) ==========")
                    ...
                    if (episode + 1) % 10 == 0:
                        ...
                        logger.info(f"  Recent 10 episodes average reward: {avg_reward:.2f}")
                        logger.info(f"  Replay buffer size: {len(fosqddpg.replay_buffer)}")
                ...
                logger.error("FOSQDDPG algorithm is not available, please check import")
                logger.info("Fallback to FOMAPPO algorithm")
                self._train_fomappo_agents_integrated()
    ...
    def _train_custom_agents(self):
        """Train custom RL agents"""
        logger.info(f"Using custom algorithm {self.rl_algorithm} for training")
            ...
            if user_id in self.rl_agents[self.rl_algorithm]:
                ...
                try:
                    ...
                    if "fomappo" in self.rl_agents and user_id in self.rl_agents["fomappo"]:
                        logger.info(f"Trying to use FOMAPPO as fallback algorithm")
                        self._train_fomappo_agents_integrated()
    ...
    def run_pipeline(self):
        ...
        if not hasattr(self, 'training_history') or not self.training_history:
            logger.warning("Training history does not exist, initializing empty training history")
            self.training_history = {
                "episode_rewards": {},
                "episode_lengths": {},
                "training_loss": {},
        ...
        if self.rl_algorithm == "fomappo":
            if not hasattr(self, 'multi_agent_env') or self.multi_agent_env is None:
                logger.warning("multi_agent_env does not exist, creating new environment")
                try:
                    # Import multi-agent environment
                    from fo_generate.multi_agent_env import MultiAgentFlexOfferEnv
            ...
            # Get actual observation dimension for subsequent adapter creation
            actual_obs_dim = None
            ...
            if not hasattr(self, 'fomappo_adapter') or self.fomappo_adapter is None:
                logger.warning("fomappo_adapter does not exist, creating new adapter")
                try:
                    ...
                    if actual_obs_dim is not None:
                        ...
                        logger.warning(f"⚠️ Using fallback method to get state dimension: {state_dim}")
            ...
            # If both exist but dimensions are inconsistent, perform dimension adaptation
                ...
                if actual_obs_dim != self.fomappo_adapter.state_dim:
                    logger.warning(f"⚠️ Detected dimension mismatch: adapter={self.fomappo_adapter.state_dim} dimensions, environment={actual_obs_dim} dimensions")
                    try:
                        ...
                        logger.error(f"❌ Failed to rebuild fomappo_adapter: {e}")
                        import traceback
                        logger.error(traceback.format_exc())
        ...
        for timestep in range(self.steps_per_episode):
            logger.info(f"\n========== Time step {timestep} (hour {timestep}) ==========")
            
            # Step 1: Update current time step user demand (incremental stacking)
            ...
            # Record current time step results
            timestep_result = {
                "timestep": timestep,
            ...
            logger.info(f"Time step {timestep} (hour {timestep}) completed: {len(fo_systems)} FO systems, {len(trade_results)} trades, {len(disaggregated_results)} disaggregated results")
    ...
    def _initialize_user_states(self):
        ...
        logger.info("Initializing user states...")
        ...
        if hasattr(self, 'users_distribution') and self.users_distribution:
            ...
            logger.info(f"Using pre-set user distribution: {user_distribution}, total number of users: {sum(user_distribution)}")
        else:
            # Fallback to standard distribution (this should not happen)
            user_distribution = [6, 10, 8, 12]  # Manager 1: 6 users, Manager 2: 10 users, Manager 3: 8 users, Manager 4: 12 users
            logger.warning(f"No pre-set user distribution found, using standard distribution: {user_distribution}")
        ...
        for manager_idx, user_count in enumerate(user_distribution):
            ...
            for local_user_idx in range(user_count):
                ...
                if global_user_idx >= self.num_users:
                    logger.warning(f"User index {global_user_idx} out of range, skipping")
                    continue
                ...
                for hour in range(self.time_horizon):
                    ...
                    # User individual difference (randomness)
            ...
            # Calculate total demand for Manager
        ...
        logger.info(f"Peak demand period: hour {peak_hour}, demand {peak_demand:.2f} kWh")
        
        # Set demand for scheduler manager
        if hasattr(self, 'schedule_manager') and self.schedule_manager:
            try:
                ...
                logger.info("User demands set for scheduler manager")
            except Exception as e:
                logger.warning(f"Error setting scheduler manager demands: {e}")
        
        return True
    
    def _update_user_demands_for_timestep(self, timestep):
        """Update user demand state for specified time step"""
        logger.info(f"Updating user demand state for time step {timestep}...")
        
        # User demand is already set during initialization, here we only need to update scheduler state
        if timestep >= self.time_horizon:
            logger.warning(f"Time step {timestep} exceeds time horizon {self.time_horizon}")
            return
        ...
        # Display current time step demand statistics
    ...
    def _generate_flexoffers_with_fomodelbased(self, timestep, fomodelbased_agents):
        """Generate FlexOffers for a specific timestep using the FOModelBased algorithm"""
        fo_systems = {}  # Use nested dictionary: manager_id -> {device_id: fo_system}
        
        # Output debug information
        print(f"\n🔍 FOModelBased generating FlexOffers - timestep {timestep}")
        logger.info(f"Using FOModelBased to generate FlexOffers for timestep {timestep}...")
        ...
        # Generate FlexOffers for each Manager using FOModelBased (deprecated)
        for manager in self.managers:
            manager_id = manager.manager_id
            ...
            if manager_id in fomodelbased_agents:
                    ...
                    for device in user.devices:
                        ...
                        if 'BATTERY' in device_type_str:
                            ...
                            try:
                                ...
                                print(f"      ✗ Battery device {device_id} parameter retrieval failed: {e}")
                                device_states[device_id] = {
                            ...
                            try:
                                ...
                                print(f"      ✗ Heat pump device {device_id} parameter retrieval failed: {e}")
                                device_states[device_id] = {
                        ...
                        # Can add other device types
                
                # Update device states in the policy
                ...
                for user in manager.users:
                    for device in user.devices:
                        ...
                        try:
                            # For battery device - fix: now handle string type of device_type
                            device_type_str = str(device_type)
                            ...
                            if 'BATTERY' in device_type_str:
                                ...
                                try:
                                    if hasattr(self, 'price_loader'):
                                        ...
                                        for t in range(self.time_horizon):
                                            ...
                                            if 0 <= hour < 6 or 22 <= hour < 24:  
                                                inv_prices.append(0.8)  
                                            elif 10 <= hour < 16:  
                                                inv_prices.append(0.2)  
                                            else:
                                                inv_prices.append(0.5)  
                                    ...
                                    logger.warning(f"Failed to generate smart battery energy profile: {e}, using default strategy")
                                    profile = []
                                    for t in range(self.time_horizon):
                                ...
                                time_flex = max(1, int(temp_diff / 2))  
                                ...
                                for t in range(self.time_horizon):
                                    ...
                                    if power > 0:
                                        curr_temp += power * 0.1  # Each kW increases 0.1 degree
                                    else:
                                ...
                                for t in range(self.time_horizon):
                                    hour = (timestep + t) % 24
                                    if 0 <= hour < 6:  # Nighttime charging
                                ...
                                max_power = -5.0  # Negative value represents generation
                                min_power = 0.0
                                ...
                                profile = [0.0] * self.time_horizon  # Default no power
                            ...
                            logger.warning(f"Failed to create DFO system for device {device_id}: {e}, using default value")
        ...
        logger.info(f"FOModelBased generated {sum(len(devices) for devices in fo_systems.values())} FlexOffer systems at time step {timestep}")
        return fo_systems
    ...
    def _generate_basic_flexoffers_for_timestep(self, timestep):
        ...
        logger.info(f"Generating basic FlexOffer for time step {timestep}...")
            ...
            for user in manager.users:
                for device in user.devices:
                    ...
                    basic_fo = {
                        ...
                        'time_horizon': self.time_horizon,
                        'timestep': timestep,
                        'flexibility_factor': 0.5  # Default flexibility
                    }
    ...
    def _generate_flexoffers_for_timestep(self, timestep):
        """Generate FlexOffer for specified time step"""
        logger.info(f"Generating FlexOffer for time step {timestep}...")
        
        if self.rl_algorithm == "fomodelbased":
            logger.info(f"Using FOModelBased algorithm to generate FlexOffer for time step {timestep}...")
            ...
            if hasattr(self, 'rl_agents') and 'fomodelbased' in self.rl_agents:
                ...
                total_fo_count = sum(len(devices) for devices in fo_systems.values())
                logger.info(f"FOModelBased algorithm generated {total_fo_count} FlexOffer systems for time step {timestep}")
            ...
            try:
                ...
                logger.info(f"FOMAPPO algorithm generated {len(fo_systems)} FlexOffer systems for Manager at time step {timestep}")
                for manager_id, dfo_dict in fo_systems.items():
                ...
                logger.warning(f"FOMAPPO fallback to environment default generation, generated {len(fo_systems)} FlexOffer systems for Manager at time step {timestep}")
                return fo_systems
            ...
            logger.warning(f"FOMAPPO algorithm not trained, using random strategy to generate {len(fo_systems)} FlexOffer systems for Manager at time step {timestep}")
            for manager_id, dfo_dict in fo_systems.items():
                logger.info(f"Manager {manager_id} generated {len(dfo_dict)} FlexOffer systems for devices")
            ...
            try:
                ...
                logger.info(f"FOMADDPG adapter generated {len(fo_systems)} FlexOffer systems for Manager at time step {timestep}")
                for manager_id, dfo_dict in fo_systems.items():
                    logger.info(f"Manager {manager_id} generated {len(dfo_dict)} FlexOffer systems for devices")
                ...
                logger.warning(f"FOMADDPG adapter fallback to environment default generation, generated {len(fo_systems)} FlexOffer systems for Manager at time step {timestep}")
                return fo_systems
            ...
            obs = self.multi_agent_env._get_observations()  # 🔧 Fix: use standardized observations
            ...
            logger.info(f"FOMADDPG original agent generated {len(fo_systems)} FlexOffer systems for Manager at time step {timestep}")
            for manager_id, dfo_dict in fo_systems.items():
                logger.info(f"Manager {manager_id} generated {len(dfo_dict)} FlexOffer systems for devices")
            ...
            if hasattr(self, 'fomatd3_adapter'):
                ...
                # Use adapter's TD3 double Critic network to select actions
                ...
                logger.info(f"FOMATD3 adapter generated {len(fo_systems)} FlexOffer systems for Manager at time step {timestep}")
                ...
                # Convert observations to numpy array (TD3 may need to handle different lengths of observations)
                try:
                    states = np.array([obs[manager_id] for manager_id in manager_ids])
                except ValueError:
                    # If observation lengths are inconsistent, pad to the same length
                    max_obs_len = max(len(obs[mid]) for mid in manager_ids)
                    states = []
                    for manager_id in manager_ids:
                        obs_array = obs[manager_id]
                ...
                # Use trained FOMATD3 agent to select actions
                actions = self.fomatd3_agent.select_actions(states, add_noise=False)
                
                # Convert actions to environment expected format
                action_dict = {manager_ids[i]: actions[i] for i in range(len(manager_ids))}
                ...
                logger.info(f"FOMATD3 original agent generated {len(fo_systems)} FlexOffer systems for Manager at time step {timestep}")
            else:
                logger.warning("No FOMATD3 agent or adapter available, using data-based method to generate FlexOffer")
                fo_systems = {}
                for manager in self.managers:
                    fo_systems[manager.manager_id] = manager.generate_dfo(self.time_horizon)
                logger.info(f"Data-based method generated {len(fo_systems)} FlexOffer systems for Manager at time step {timestep}")
                return fo_systems
            for manager_id, dfo_dict in fo_systems.items():
                logger.info(f"Manager {manager_id} generated {len(dfo_dict)} FlexOffer systems for devices")
            ...
            # Use FOSQDDPG adapter to generate FlexOffer
            try:
                obs = self.multi_agent_env._get_observations()
                
                # Use trained FOSQDDPG adapter to select actions (using Shapley Q-values)
                ...
                logger.info(f"FOSQDDPG adapter generated {len(fo_systems)} FlexOffer systems for Manager at time step {timestep}")
                for manager_id, dfo_dict in fo_systems.items():
                    logger.info(f"Manager {manager_id} generated {len(dfo_dict)} FlexOffer systems for devices")
                ...
                logger.warning(f"FOSQDDPG adapter fallback to environment default generation, generated {len(fo_systems)} FlexOffer systems for Manager at time step {timestep}")
                return fo_systems
        elif self.rl_algorithm == "fosqddpg" and hasattr(self, 'multi_agent_env') and hasattr(self, 'fosqddpg_agent'):
            # Use FOSQDDPG algorithm to generate FlexOffer
            obs = self.multi_agent_env._get_observations() 
            manager_ids = list(obs.keys())
            
            # Convert observations to numpy array (SQDDPG may need to handle different lengths of observations)
            try:
                states = np.array([obs[manager_id] for manager_id in manager_ids])
            except ValueError:
                # If observation lengths are inconsistent, pad to the same length
            ...
            # Use trained FOSQDDPG agent to select actions
            ...
            logger.info(f"FOSQDDPG algorithm generated {len(fo_systems)} FlexOffer systems for Manager at time step {timestep}")
            for manager_id, dfo_dict in fo_systems.items():
                logger.info(f"Manager {manager_id} generated {len(dfo_dict)} FlexOffer systems for devices")
            
            return fo_systems
        elif self.rl_algorithm == "fomaippo" and hasattr(self, 'multi_agent_env') and hasattr(self, 'independent_fomappo_adapter'):
            # Use FOMAIPPO algorithm to generate FlexOffer
            obs = self.multi_agent_env._get_observations()
            
            # Use trained FOMAIPPO adapter to select actions
            try:
                ...
                logger.info(f"FOMAIPPO algorithm generated {len(fo_systems)} FlexOffer systems for Manager at time step {timestep}")
                for manager_id, dfo_dict in fo_systems.items():
                    logger.info(f"Manager {manager_id} generated {len(dfo_dict)} FlexOffer systems for devices")
                ...
                logger.warning(f"FOMAIPPO fallback to environment default generation, generated {len(fo_systems)} FlexOffer systems for Manager at time step {timestep}")
                return fo_systems
        else:
            # For other algorithms or untrained cases, use multi-agent environment to generate FlexOffer
            if hasattr(self, 'multi_agent_env'):
                try:
                    obs = self.multi_agent_env._get_observations()  
                    ...
                    logger.info(f"{self.rl_algorithm} algorithm used multi-agent environment to generate FlexOffer, generated {len(fo_systems)} FlexOffer systems for Manager at time step {timestep}")
                    return fo_systems
                    
                except Exception as e:
                    logger.error(f"Multi-agent environment FlexOffer generation failed: {e}")
                    logger.warning(f"{self.rl_algorithm} algorithm fallback to basic generation method")
                    return self._generate_basic_flexoffers_for_timestep(timestep)
            else:
                if self.rl_algorithm == "fomappo":
                    logger.info(f"Using basic generation method to generate FlexOffer for {self.rl_algorithm} algorithm")
                else:
                    logger.warning(f"{self.rl_algorithm} algorithm does not support time step level FlexOffer generation, using basic generation method")
                    return self._generate_basic_flexoffers_for_timestep(timestep)
    
    def _aggregate_flexoffers_for_timestep(self, fo_systems, timestep):
        """Aggregate FlexOffer for specified time step"""
        logger.info(f"Aggregating FlexOffer for time step {timestep}...")
        
        if not fo_systems:
            logger.warning(f"No FlexOffer to aggregate for time step {timestep}")
            return []
            ...
            for device_id, fo_system in manager_systems.items():
                ...
                if hasattr(fo_system, 'current_fo') and fo_system.current_fo:
                    ...
                    try:
                        ...
                        logger.warning(f"Failed to convert DFO system {device_id}: {e}")
                        continue
                        
        if not flex_offers:
            logger.warning(f"No valid FlexOffer for time step {timestep}")
            return []
        
        # Use new aggregator to aggregate
        try:
            aggregated_results = self.fo_aggregator.aggregate(flex_offers)
            logger.info(f"Time step {timestep} FlexOffer aggregation completed, generated {len(aggregated_results)} aggregated results")
    ...
    def _convert_dfo_to_flexoffer(self, dfo_system, device_id, timestep):
        ...
        try:
            ...
            # Create FOSlice for each DFO slice (only for real DFO system)
            if slices:
                for i, dfo_slice in enumerate(slices):
                # Calculate start and end time of time slice
                    slice_start = base_time + timedelta(hours=hour, minutes=i*2)  # Each slice is 2 minutes
            ...
            # If DFO system has no slices, create a default slice
            if not fo_slices:
                ...
                fo_slice = FOSlice(
                    ...
                    duration_minutes=60.0,  # 60 minutes
                    device_type=device_type,
                    device_id=device_id,
                    priority=3,
            ...
            fo = FlexOffer(
                ...
                hour=hour % 24,  # Ensure hour is in 0-23 range
            ...
            logger.error(f"Failed to convert DFO system {device_id} to FlexOffer: {e}")
            import traceback
            logger.error(traceback.format_exc())
            return None
    ...
    def _trade_flexoffers_for_timestep(self, aggregated_results, timestep):
        ...
        logger.info(f"Trading FlexOffer for time step {timestep}...")
        
        if not aggregated_results:
            logger.warning(f"No aggregated results to trade for time step {timestep}")
            return []
        ...
        # Assign aggregated results to each Manager
        for i, manager in enumerate(self.managers):
            manager_id = manager.manager_id
            
            # Assign an aggregated result to each Manager, if results are insufficient, cycle through them
        ...
        # Ensure each Manager has a trading opportunity
        ...
        logger.info(f"Time step {timestep} trading completed, executed {len(trades)} trades")
        logger.info(f"Buyers involved in trading: {set(trade.buyer_id for trade in trades)}")
        logger.info(f"Sellers involved in trading: {set(trade.seller_id for trade in trades)}")
    ...
    def _disaggregate_flexoffers_for_timestep(self, trade_results, original_fo_systems, timestep):
        ...
        logger.info(f"Disaggregating FlexOffer for time step {timestep}, using algorithm: {self.disaggregation_method}...")
        
        if not trade_results:
            logger.warning(f"No trade results to disaggregate for time step {timestep}")
            return []
        ...
        for trade in trade_results:
            ...
            if buyer_data:
                try:
                    ...
                    # Call real disaggregate algorithm
                    ...
                    for result_data in disaggregate_results:
                        disaggregated_result = {
                            ...
                            'weight_ratio': result_data.get('weight_ratio', 1.0)  # Only proportional algorithm has weight ratio
                        }
                        disaggregated_results.append(disaggregated_result)
                    
                except Exception as e:
                    logger.error(f"Disaggregate algorithm call failed: {e}, fallback to simple average distribution")
                    ...
                    logger.info(f"Fallback processing: disaggregate trade {trade.trade_id} for {buyer_id}: {trade_quantity:.2f} kWh average distributed to {len(buyer_data)} devices")
            else:
                default_result = {
                    'manager_id': buyer_id,
        ...
        logger.info(f"Time step {timestep} disaggregation completed, generated {len(disaggregated_results)} disaggregation results")
    ...
    def _schedule_and_update_states(self, disaggregated_results, timestep):
        ...
        logger.info(f"Scheduling and updating user states for time step {timestep}...")
        
        # Process disaggregation results, update user states
        ...
        # Allocate allocated energy to users
        total_allocated = 0
        for manager in self.managers:
            manager_id = manager.manager_id
            ...
            if allocated_energy > 0:
                # Allocate energy equally to users of Manager
                        ...
                        try:
                            if 'manager_' in user_id:
                                ...
                                if len(parts) >= 4:
                                    ...
                                    # Calculate global user index based on user distribution
                                    user_distribution = [6, 10, 8, 12]  # Manager 1:6 users, Manager 2:10 users, Manager 3:8 users, Manager 4:12 users
        ...
        logger.info(f"Time step {timestep}: allocated {total_allocated:.2f} kWh energy, average user satisfaction: {avg_satisfaction:.3f}")
    ...
    def _save_rewards_to_csv(self, csv_file, rewards_data, algorithm_name):
        ...
        try:
            ...
            if rows:
                df = pd.DataFrame(rows)
                df.to_csv(csv_file, index=False)
                logger.info(f"{algorithm_name} reward data saved to {csv_file}, total {len(rows)} rows")
            else:
    ...
    def _calculate_pipeline_execution_rewards(self, results):
        ...
        try:
            ...
            for timestep_data in timestep_results:
                ...
                satisfaction_reward = user_satisfaction * 100.0  # 0-100 points, greatly amplified
                
                # 3. Trade success rate reward
                trades_count = len(trades)
                coordination_reward = min(trades_count * 20.0, 100.0)  # 20 points per trade, highest 100 points
                
                # 4. Disaggregation efficiency reward
                disaggregated_count = len(timestep_data.get("disaggregated_results", []))
                efficiency_reward = min(disaggregated_count * 2.0, 50.0)  # 2 points per disaggregation result, highest 50 points
                
                # Trade value reward
                trade_value_reward = trade_value * 100.0  # Trade value multiplied by 100 as reward
                
                # Calculate total reward for timestep
                timestep_reward = {
                    'timestep': timestep,
                    'trade_value': trade_value,
                    'trade_value_reward': trade_value_reward,  # New trade value reward
                ...
                # Allocate timestep reward to each manager
    ...
    def _save_pipeline_rewards_history(self, pipeline_rewards):
        ...
        try:
            ...
            with open(csv_file, 'w', newline='', encoding='utf-8') as f:
                ...
                if total_episodes > 0:
                    ...
                    writer.writerow([
                        ...
                        float(total_trade_value * 100.0),  # Total trade value reward
            ...
            # Save Pipeline reward as extra information, not overwrite training history
            if hasattr(self, 'training_history') and isinstance(self.training_history, dict):
                # Save Pipeline reward as extra information
                ...
                if not self.training_history.get('episode_rewards') or (
                    ...
                    logger.warning("⚠️ Training history is empty, use Pipeline reward as fallback")
                    ...
                    logger.info("✅ Keep original training history, Pipeline reward saved as extra information")
                    self.training_history['data_source'] = 'training_episodes'
                
                # Save training history
                try:
                    self._save_training_history_to_csv(algorithm_name)
                    logger.info("✅ Training history saved (keep original training data)")
                except Exception as e:
    ...
    def _save_pipeline_results_to_csv(self, csv_file, results, algorithm_name):
        ...
        Args:
            csv_file: CSV file path
            results: Pipeline execution results
            algorithm_name: Algorithm name used
        """
        try:
            import pandas as pd
            
            # Time step level results
            timestep_rows = []
            for timestep_result in results["timestep_results"]:
                ...
                # Count trades and disaggregation results for this time step
            ...
            if timestep_rows:
                ...
                df.to_csv(csv_file, index=False)
                logger.info(f"Pipeline execution results saved to {csv_file}, total {len(timestep_rows)} rows")
            else:
    ...
    def _train_fomappo_agents_integrated(self):
        """FOMAPPO training based on original MAPPO shared/base_runner.py mode - real MAPPO process"""
        ...
        logger.info("🔧 Start FOMAPPO training based on original MAPPO mode")
        ...
        try:
            ...
            # 3. Initialize FOMAPPO adapter (shared policy)
            try:
                from algorithms.MAPPO.fomappo.fomappo_adapter import FOMAPPOAdapter
                self.fomappo_adapter = FOMAPPOAdapter(
                    state_dim=state_dim,
                ...
                logger.warning("FOMAPPOAdapter not available, use original training method")
            ...
            try:
                ...
                if os.path.exists(training_csv_file):
                    logger.info(f"✅ Real-time save file verification successful: {training_csv_file}")
                else:
                    logger.error(f"❌ Real-time save file creation failed: {training_csv_file}")
            except Exception as e:
                logger.error(f"Failed to create real-time training history file: {e}")
            
            # 5. Training loop - according to original MAPPO shared/base_runner.py mode
            episodes = self.num_episodes
            logger.info(f"Start {episodes} episodes of MAPPO-style training")
            
            for episode in range(episodes):
                logger.info(f"\n========== Episode {episode+1}/{episodes} (MAPPO-style FOMAPPO) ==========")
                
                # ===== MAPPO mode: warmup - reset environment =====
                obs, infos = multi_env.reset()
                self.fomappo_adapter.reset_buffer()
                
                episode_rewards = {manager_id: 0.0 for manager_id in manager_ids}
                
                # ===== MAPPO mode: for step in range(self.episode_length) =====
                for step in range(self.steps_per_episode):
                    logger.info(f"Episode {episode+1}, time step {step} (hour {step})")
                    
                    # ===== MAPPO mode: collect(step) - collect one step data =====
                    actions, action_log_probs, values = self.fomappo_adapter.select_actions(obs, deterministic=False)
                    
                    # ===== MAPPO mode: envs.step(actions) - environment step =====
                    next_obs, rewards, dones, truncated, infos = multi_env.step(actions)
                    
                    # ===== MAPPO mode: insert(data) - insert data into buffer =====
                    ...
                    logger.info(f"  Time step {step}: total reward {sum(rewards.values()):.3f}")
                
                # ===== MAPPO mode: compute() - calculate returns and advantages =====
                self.fomappo_adapter.compute_returns()
                
                # ===== MAPPO mode: train() - train network (multiple epochs + mini-batch) =====
                train_info = self.fomappo_adapter.train_on_batch()
                
                # Record episode rewards and save in real-time
                for manager_id in manager_ids:
                    total_rewards[manager_id].append(episode_rewards[manager_id])
                
                # Save to CSV in real-time (ensure data is not lost)
                try:
                    import csv
                    # Add debug information
                ...
                # ===== MAPPO mode: save model periodically =====
            ...
            # Verify training history data completeness
            ...
            for manager_id, rewards in total_rewards.items():
                logger.info(f"{manager_id}: {len(rewards)} episodes, sample: {rewards[:3] if rewards else 'Empty'}")
            ...
            if not has_valid_data:
                ...
                # Create test data to ensure save functionality
                for manager_id in manager_ids:
            ...
            # Save trained components to instance variables (adapter is already self.fomappo_adapter)
            self.multi_agent_env = multi_env
            ...
            # Enhance training history save method
            logger.info("💾 Start saving training history...")
            
            # Method 1: Use main CSV save method
            try:
                # Force ensure data exists and save directly
                logger.info(f"🔍 Final check before save: self.training_history['episode_rewards'] = {type(self.training_history['episode_rewards'])}")
                if isinstance(self.training_history["episode_rewards"], dict):
                    for k, v in self.training_history["episode_rewards"].items():
                        logger.info(f"  Check before save {k}: {len(v)} episodes")
                ...
                if not self.training_history["episode_rewards"] or (isinstance(self.training_history["episode_rewards"], dict) and not any(len(rewards) > 0 for rewards in self.training_history["episode_rewards"].values())):
                    logger.warning("⚠️ Training history is empty, use total_rewards to overwrite")
                    self.training_history["episode_rewards"] = total_rewards
            ...
            # Method 2: Use backup save method
            try:
            ...
            # Method 3: Save raw data directly to JSON (ensure data is recorded)
            try:
                import json
                timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
            ...
            # Method 4: Save CSV file directly
            try:
                import csv
                ...
                with open(csv_file, 'w', newline='') as f:
                    ...
                    if data_to_save:
                        ...
                        logger.warning("💾 Creating minimal training history data")
                        for i in range(4):  # 4 managers
                            manager_id = f"manager_{i+1}"
                            for episode in range(min(10, self.num_episodes)):  # Save at least 10 episodes
                ...
                # Ensure at least one empty training history file is created
                try:
            ...
            for manager_id in manager_ids:
                ...
                if len(rewards) >= 20:
                    ...
                    logger.info(f"{manager_id}: First 10 episodes average {first_10_avg:.3f} → Last 10 episodes average {last_10_avg:.3f} (improvement {improvement:+.3f})")
                else:
            ...
            logger.info("🎉 Fully implemented in the original MAPPO shared/base_runner.py mode!")
            logger.info("==========================================")
            
            # 🔧 Final insurance measure: force save training history
            logger.info("🛡️ Execute final force save insurance measure...")
            try:
            ...
            import traceback
            logger.error(traceback.format_exc())
            logger.info("Roll back to original training method")
            return self._train_fomappo_agents_original()
            
    def _train_fomappo_agents_fixed(self):
        logger.info("Start training using the correct fixed FOMAPPO adapter")
        ...
        try:
            ...
            try:
                from algorithms.MAPPO.fomappo.fomappo_adapter import FOMAPPOAdapter
                logger.info("✅ Successfully imported standard FOMAPPO adapter (shared policy architecture)")
                use_correct_adapter = True
            except ImportError as e:
                logger.error(f"Failed to import standard FOMAPPO adapter: {e}")
                logger.info("Roll back to original method")
            ...
            # 5. Training loop - use standard MAPPO data stream
            total_rewards = {manager_id: [] for manager_id in manager_ids}
            ...
            for episode in range(self.num_episodes):
                ...
                # Collect data for a complete episode according to standard MAPPO process
                for timestep in range(self.steps_per_episode):
                    ...
                    logger.info(f"  Timestep {timestep}: Total reward {sum(rewards.values()):.3f}")
                
                # Calculate returns and advantages after episode
                adapter.compute_returns()
                
                # Use standard MAPPO training method
            ...
            logger.info(f"Total training iterations: {total_training_iterations}")
            if use_correct_adapter:
                logger.info("🎉 Using correct fixed FOMAPPO, action_log_probs problem solved!")
            else:
                logger.warning("⚠️ Using old fixed FOMAPPO, still have action_log_probs problem")
            logger.info("==========================================")
            
        except Exception as e:
            logger.error(f"Correct fixed FOMAPPO training failed: {e}")
            import traceback
            logger.error(traceback.format_exc())
            logger.info("Roll back to original FOMAPPO training method")
            
            # Roll back to original method
            return self._train_fomappo_agents_integrated()
    ...
    def _train_fomaippo_agents(self):
        ...
        try:
            ...
            print("✅ Successfully imported train_fomaippo_independent_policy")
            logger.info("✅ Successfully imported train_fomaippo_independent_policy, call independent policy training method")
            result = train_fomaippo_independent_policy(self)
            print("✅ External training method executed")
            ...
            if isinstance(result, dict) and result.get('status') == 'success':
                ...
                logger.info(f"Verify: hasattr(self, 'independent_fomaippo_adapter') = {hasattr(self, 'independent_fomaippo_adapter')}")
        ...
        try:
            ...
            if not FOMAIPPO_available or FOMAIPPOAdapter is None:
                logger.error("❌ FOMAIPPO is not available, roll back to original method")
                return self._train_fomappo_agents_integrated()
            ...
            for episode in range(self.num_episodes):
                ...
                for timestep in range(self.steps_per_episode):
                    ...
                    # Step 1: independent policy select actions
            ...
            try:
                ...
                logger.info(f"✅ Training history record verification completed: {len(episode_rewards_dict)} managers, each {self.num_episodes} episodes")
            except Exception as e:
            ...
            # Use multiple save methods to ensure data is not lost
            # Method 1: Main CSV save method
            try:
            ...
            logger.info("🎉 Advantage: each manager independently learns, avoids policy conflicts, and improves learning efficiency!")
            logger.info("==========================================")
    ...
    def _train_fomaddpg_agents_optimized(self):
        """Train FOMADDPG multi-agent algorithm - optimized version, solving overfitting and training instability problems"""
        print("\n🔧 Entering FOMADDPG training method...")
        logger.info("🔧 Starting _train_fomaddpg_agents_optimized method")
        ...
        try:
            ...
            if isinstance(result, dict) and result.get('status') == 'success':
                ...
                logger.info(f"Validation: hasattr(self, 'fomaddpg_adapter') = {hasattr(self, 'fomaddpg_adapter')}")
        ...
        print("\n🚀 Roll back to original FOMADDPG training (solving overfitting and instability problems)")
        logger.info("🚀 Roll back to original FOMADDPG training (solving overfitting and instability problems)")
        ...
        try:
            # Check if FOMADDPG adapter is available
            if not FOMADDPG_available or FOMAddpgAdapter is None:
                logger.error("❌ FOMAddpgAdapter is not available, roll back to original method")
            ...
            for episode in range(self.num_episodes):
                ...
                if episode > WARMUP_EPISODES:
                    ...
                    # Update noise parameters of the adapter
                ...
                for timestep in range(self.steps_per_episode):
                    ...
                    # Step 1: Use adapter to select actions
                    # Optimization 7: Use more exploration in the early stages
                    use_noise = episode < WARMUP_EPISODES * 2  # Use noise in the first 100 episodes
                    ...
                    # Optimization 8: Control training frequency, avoid overtraining
                    should_train = (
                        episode >= WARMUP_EPISODES and  # After warmup period
                ...
                # Record training loss values (FOMADDPG uses actor_loss and critic_loss)
                if train_info:
                    adjusted_train_info = {
                ...
                # Optimization 9: Progress monitoring
                if (episode + 1) % 10 == 0:
                    ...
                    for manager_id in manager_ids:
                        ...
                        if len(recent_rewards) >= 10:
                            ...
                            # Check if converged
                            if recent_std < 5.0:  # Reward variance less than 5 is considered converged
            ...
            try:
                ...
                logger.info(f"✅ Optimized training history verification completed: {len(episode_rewards_dict)} managers, {self.num_episodes} episodes")
            except Exception as e:
                logger.warning(f"Save training history failed: {e}")
            ...
            try:
                ...
                logger.info("✅ Optimized FOMADDPG force save completed")
            except Exception as e:
                logger.error(f"Force save failed: {e}")
            ...
            # Output optimized training statistics
            logger.info(f"\n========== Optimized FOMADDPG training summary ==========")
            
            try:
                final_stats = fomaddpg_adapter.get_training_stats()
                
                logger.info("🎯 Optimized training analysis:")
                for manager_id in manager_ids:
                    rewards = episode_rewards_dict[manager_id]
                    if len(rewards) >= 100:
                        # Compare average rewards of the first 50 and last 50 episodes
                        early_avg = np.mean(rewards[50:100])   # Average reward of the first 50 episodes
                        late_avg = np.mean(rewards[-50:])      # Average reward of the last 50 episodes
                        improvement = late_avg - early_avg
                        stability = np.std(rewards[-50:])      # Stability of the last 50 episodes
                ...
                if isinstance(final_stats, dict):
                    ...
                    buffer_size = final_stats.get('buffer_size', 0)
                    logger.info(f"🚀 Total training iterations: {total_iterations} (optimized significantly)")
                    logger.info(f"📦 Final experience buffer size: {buffer_size}")
                ...
                logger.warning(f"Get final statistics failed: {e}")
                logger.info("🎯 Optimized training completed, statistics information retrieval failed")
            
            logger.info("🎉 Optimized highlights:")
            ...
            import traceback
            logger.error(traceback.format_exc())
            logger.info("Roll back to original FOMADDPG algorithm")
            
            # Roll back to original method
            return self._train_fomaddpg_agents()

    def _train_fomatd3_agents_with_adapter(self):
        """Use FOMATD3 adapter for training - imitate FOMADDPG's success pattern"""
        ...
        try:
            ...
            logger.info("✅ Successfully imported train_fomatd3_adapter, calling optimized training method")
            result = train_fomatd3_adapter(self)
            print("✅ External training method execution completed")
            
            # Process the object returned by the external training method
            if isinstance(result, dict) and result.get('status') == 'success':
                logger.info("✅ External training method completed, setting adapter reference")
                if 'multi_agent_env' in result:
                    self.multi_agent_env = result['multi_agent_env']
                    logger.info("✅ Set multi_agent_env")
                ...
                logger.info(f"Validation: hasattr(self, 'multi_agent_env') = {hasattr(self, 'multi_agent_env')}")
                logger.info(f"Validation: hasattr(self, 'fomatd3_adapter') = {hasattr(self, 'fomatd3_adapter')}")
                ...
                logger.warning("External training method returned unexpected result format, trying internal implementation")
                
        except Exception as e:
            logger.error(f"❌ Import or execution of external training method failed: {e}")
            import traceback
            logger.error(f"Exception details: {traceback.format_exc()}")
            print(f"❌ External training method failed, rolling back to internal implementation: {e}")
        
        # If external method fails, use internal implementation (original code)
        print("\n🚀 Rolling back to original FOMATD3 training method (based on TD3 double Critic network)")
        logger.info("🚀 Rolling back to original FOMATD3 training method")
        ...
        try:
            ...
            if not FOMATD3_available or FOMATD3Adapter is None:
                logger.error("❌ FOMATD3Adapter not available, rolling back to original method")
            ...
            for episode in range(self.num_episodes):
                ...
                # Reset environment
                obs, infos = multi_env.reset()
                # 🔧 Fix 2: remove reset_buffers() call, keep valuable experience in replay buffer
                # fomatd3_adapter.reset_buffers()  # Commented out! The replay buffer should maintain historical experience
                ...
                for timestep in range(self.steps_per_episode):
                    ...
                    # TD3 features: training updates can be performed at each step (if there is enough experience)
                    # 🔧 Fix 4: Start training from step 0, no longer delay
                    ...
                    if train_info and train_info.get('actor_loss', 0) > 0:
                        ...
                        if timestep == 0:  # Only display warm-up information on the first step
                ...
                if train_info and train_info.get('status') != 'warming_up':
                    adjusted_train_info = {
                        'policy_loss': train_info.get('actor_loss', 0.0),
                        'value_loss': train_info.get('critic_loss', 0.0),
                        'entropy': 0.0  # TD3 usually has no entropy loss
                    }
                    self._record_training_loss_for_all_managers(episode, adjusted_train_info, manager_ids)
            ...
            try:
                ...
                logger.info(f"✅ Training history verification completed: {len(episode_rewards_dict)} managers, {self.num_episodes} episodes each")
            except Exception as e:
                logger.warning(f"Save training history failed: {e}")
                episode_rewards_dict = {manager_id: [0.0] * self.num_episodes for manager_id in manager_ids}
            ...
            import traceback
            logger.error(traceback.format_exc())
            logger.info("Rolling back to original FOMATD3 algorithm")
    ...
    def _train_fosqddpg_agents_with_adapter(self):
        ...
        print("\n🔧 Entering FOSQDDPG training method...")
        logger.info("🔧 Starting _train_fosqddpg_agents_with_adapter method")
        ...
        try:
            ...
            logger.info("✅ Successfully imported train_fosqddpg_adapter, calling optimized training method")
            result = train_fosqddpg_adapter(self)
            print("✅ External training method execution completed")
            
            # Process the object returned by the external training method
            if isinstance(result, dict) and result.get('status') == 'success':
                logger.info("✅ External training method completed, setting adapter references")
                if 'multi_agent_env' in result:
                    self.multi_agent_env = result['multi_agent_env']
                    logger.info("✅ Set multi_agent_env")
        ...
        print("\n🚀 Rolling back to original FOSQDDPG training method (based on Shapley value fair credit allocation)")
        logger.info("🚀 Rolling back to original FOSQDDPG training method")
        ...
        try:
            ...
            if not FOSQDDPG_available or FOSQDDPGAdapter is None:
                logger.error("❌ FOSQDDPGAdapter not available, rolling back to original method")
            ...
            for episode in range(self.num_episodes):
                ...
                for timestep in range(self.steps_per_episode):
                    ...
                    should_train = (
                        episode >= WARMUP_EPISODES and  # After warmup period才开始训练
                ...
                # Record training loss values (FOSQDDPG uses actor_loss and critic_loss)
                if should_train and train_info:
                    adjusted_train_info = {
                        'policy_loss': train_info.get('actor_loss', 0.0),
                        'value_loss': train_info.get('critic_loss', 0.0),
                        'entropy': 0.0  # SQDDPG usually has no entropy loss
                    }
                    self._record_training_loss_for_all_managers(episode, adjusted_train_info, manager_ids)
                ...
                if (episode + 1) % 50 == 0:
                    ...
                    if len(total_rewards) >= 100:
                        ...
                        trend = "📈 Up" if improvement > 5 else "📉 Down" if improvement < -5 else "➡️ Steady"
                    ...
                    # 🔧 Periodically save model (consistent with other algorithms)
            ...
            self.training_history["total_rewards"] = total_rewards  
            ...
            self.multi_agent_env = multi_env  # 🔧 Fix: set multi-agent environment to support Pipeline execution stage
            ...
            import traceback
            logger.error(traceback.format_exc())
            logger.info("Rolling back to original FOSQDDPG algorithm")
    ...
    def run_fomodelbased_evaluation(self):
        """Run traditional optimization evaluation using FOModelBased - directly executes the complete FO Pipeline process"""
        print("\n🚀 Starting FOModelBased traditional optimization evaluation (physics-based model, no training needed)")
        logger.info("🚀 Starting FOModelBased traditional optimization evaluation (physics-based model, no training needed)")
        ...
        try:
            ...
            if not FOMODELBASED_available or FOModelBased is None:
                logger.error("❌ FOModelBased not available")
                return
            ...
            print("🔧 Initializing FOModelBased adapter...")
            logger.info("🔧 Initializing FOModelBased adapter")
            
            # Create FOModelBased agents for each Manager
            fomodelbased_agents = {}
            for manager in self.managers:
                manager_id = manager.manager_id
                ...
                # Iterate through users and devices of the Manager to generate appropriate device configurations
                for user in manager.users:
                    for device in user.devices:
                        device_id = device.device_id
                        device_type = device.device_type
                        
                        # Process device type - Fix: now handling string type device_type
                        device_type_str = str(device_type)  # Ensure it's a string
                        ...
                        if 'BATTERY' in device_type_str:
                            ...
                            device_configs[device_id] = {
                                ...
                                'params': {
                                    ...
                                    'Pmin': getattr(params, 'p_min', -3.0),  # Minimum power (negative for discharge)
                                    'Pmax': getattr(params, 'p_max', 3.0),  # Maximum power (positive for charge)
                                    'eta': getattr(params, 'efficiency', 0.95),  # Charging efficiency
                                    'decay': 1.0,  # Capacity decay factor
                            ...
                            device_configs[device_id] = {
                                ...
                                'params': {
                                    ...
                                    'T_out': 5.0,  # Assumed outdoor temperature
                                    'Pmax': getattr(params, 'max_power', 2.0),  # Maximum heating power
                                    'Area': 100.0,  # Assumed house area
                                    'c_ht': 10.0,  # Heat transfer coefficient
                                    'c': 1005.0,  # Specific heat capacity of air
                                    'm': 120.0,  # Air mass
                                    'time': 3600.0  # Time step length (seconds)
                                }
                            }
                        else:
                        ...
                        # Can add more device type handling logic
                
                # Create FOModelBased adapter
                fomodelbased_agents[manager_id] = FOModelBasedAdapter(
                    state_dim=20,  # Assumed state dimension is 20
                    action_dim=10,  # Assumed action dimension is 10
            ...
            print(f"✅ FOModelBased adapter initialization complete, total {len(fomodelbased_agents)} Managers")
            
                            # Save FOModelBased agents to rl_agents so they can be used in the pipeline
            ...
            # 🎯 Run the complete FO Pipeline process to get the actual reward
            print("🎯 Running the complete FlexOffer Pipeline process (traditional optimization)...")
            logger.info("🎯 Starting to run the complete FlexOffer Pipeline process (traditional optimization)...")
            ...
            # Run the complete Pipeline for a single episode
            episode_rewards = []
            timestep_details = []
            
            print(f"📊 Starting Pipeline evaluation (time range: {self.time_horizon} hours)...")
            
            # Execute Pipeline process
            for timestep in range(self.time_horizon):
                print(f"   📅 Timestep {timestep}/{self.time_horizon-1}")
                
                # Update user demands
                self._update_user_demands_for_timestep(timestep)
                
                # Generate FlexOffers using FOModelBased
                fo_systems = self._generate_flexoffers_for_timestep(timestep)
                total_fo_count = sum(len(devices) for devices in fo_systems.values())
                ...
                aggregated_results = self._aggregate_flexoffers_for_timestep(fo_systems, timestep)
                print(f"      🔗 Aggregation complete: {len(aggregated_results)} aggregated results")
                
                # Trade FlexOffers
                trade_results = self._trade_flexoffers_for_timestep(aggregated_results, timestep)
                total_revenue = trade_results.get('total_revenue', 0) if isinstance(trade_results, dict) else 0
                print(f"      💰 Trading complete: revenue ${total_revenue:.2f}")
                ...
                # Process rewards - if it's a dictionary, convert to rewards for each Manager
                ...
                # Create a fixed reward value for each Manager - ensure there are actual values stored in self.fomodelbased_results
                manager_rewards = {}
                ...
                for manager_id in fomodelbased_agents.keys():
                    ...
                    fo_reward = total_fo_count * 0.05  # Reward for each FlexOffer: 0.05
                    ...
                    # Add random fluctuation (±20%) to make rewards different for each Manager
                    ...
                    # If no rewards list was created for this Manager, initialize it
                    if manager_id not in pipeline_rewards:
                        pipeline_rewards[manager_id] = []
                ...
                print(f"      ⭐ Timestep reward: {timestep_reward:.4f}")
                print(f"      💰 Manager rewards: {', '.join([f'{m_id}: {r:.2f}' for m_id, r in manager_rewards.items()])}")
            
            # Calculate episode total reward
            ...
            print(f"   Average timestep reward: {np.mean(episode_rewards):.4f}")
            print(f"   Maximum timestep reward: {max(episode_rewards):.4f}")
            print(f"   Minimum timestep reward: {min(episode_rewards):.4f}")
            ...
            # Save specialized FOModelBased result format - these results will be used in _save_training_history_to_csv
            ...
            print(f"  Total timesteps: {self.time_horizon}")
            
            # Display reward statistics for each Manager
            manager_totals = {}
            for manager_id, rewards in pipeline_rewards.items():
                ...
                print(f"  {manager_id}: Total reward: {total_reward:.2f}, Average timestep reward: {avg_reward:.2f}")
            ...
            print(f"  System average timestep reward: {grand_avg:.2f}")
            ...
            # Save CSV format results - Modified to use _save_training_history_to_csv
            try:
                ...
                # Method 2: Directly save reward data
                ...
                logger.warning(f"Error saving rewards to CSV: {e}")
            ...
            print(f"🎯 Total reward: {total_episode_reward:.4f}")
            print(f"🎯 Advantages: No training required, physics-based model, deterministic results, immediately available!")
            
            logger.info("==========================================")
            logger.info(f"🎉 FOModelBased traditional optimization evaluation completed! Total reward: {total_episode_reward:.4f}")
            logger.info("🎯 Advantages: No training required, physics-based model, deterministic results, immediately available!")
            logger.info("==========================================")
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(description="FlexOffer complete process")
    ...
    # User and device configuration
    parser.add_argument("--num_users", type=int, default=36, help="Number of users (recommended 36, matching multi-agent environment)")
    parser.add_argument("--num_managers", type=int, default=4, help="Number of managers (recommended 4, matching multi-agent environment)")
    
    # Algorithm selection
    parser.add_argument("--rl_algorithm", type=str, default="fomappo", 
                        help="RL algorithm, options: 'fomappo', 'fomaddpg', 'fomatd3', 'fosqddpg' or custom algorithm name (must be registered first)")
    parser.add_argument("--aggregation_method", type=str, default="DP", choices=["LP", "DP"], help="Aggregation method: LP (Longest Profile Aggregation), DP (Dynamic Profile Aggregation)")
    ...
    parser.add_argument("--disaggregation_method", type=str, default="proportional", 
                        ...
                        help="Disaggregation method: average (Average Disaggregation, E_i=E/N), proportional (Proportional Disaggregation, E_i=(w_i/W)*E)")
    ...
    parser.add_argument("--custom_agent_name", type=str, default=None, 
                        help="Custom RL algorithm name, used for registration")
    
    # Data files
    parser.add_argument("--price_data_file", type=str, default=None, help="Electricity price data file")
    ...
    parser.add_argument("--log_verbosity", type=str, default="brief", 
                        ...
                        help="Log verbosity: minimal, brief, detailed, debug")
    
    # Test parameters
    parser.add_argument("--test_aggregation", action="store_true", help="Test differences between aggregation methods (LP and DP)")
...
def main():
    """Main function"""
    # Mark registry initialization complete to avoid duplicate registration logs in subprocesses
    RLRegistry.init()
    
    args = parse_args()
    
    # If testing aggregation methods is specified, run the test
    if args.test_aggregation:
        ...
        print("\nTest completed, exiting program")
        import sys
        sys.exit(0)
    ...
    if args.custom_agent_path and args.custom_agent_name:
        try:
            ...
            logger.info(f"Successfully loaded and registered custom RL algorithm {args.custom_agent_name}")
        except Exception as e:
    ...
    print("✅ FOPipeline object creation complete")
    ...
    if isinstance(pipeline.training_history["episode_rewards"], dict):
        print(f"Number of Managers in training history: {len(pipeline.training_history['episode_rewards'])}")
    ...
    # Ensure experiment ID has been generated (if training didn't set it)
    ...
    # 🔧 Fix: Display trading result statistics
    ...
    # Save results using the actually running algorithm name
    ...
    print("\n========== Episode Run Statistics ==========")
    print(f"Requested algorithm: {args.rl_algorithm}")
    print(f"Actually running algorithm: {actual_algorithm}")
    print(f"Completed episodes: 1 episode")
    print(f"Total timesteps: {total_timesteps} (0-{total_timesteps-1} hours)")
    print(f"Total number of trades: {total_trades}")
    print(f"Total number of disaggregation results: {total_disaggregated}")
    print(f"Total trade value: {total_trade_value:.2f} $")
    print(f"24-hour average user satisfaction: {avg_satisfaction:.3f}")
    print("====================================\n")
    
    # Save complete process results to CSV (using the actual algorithm name)
    ...
    # 🔧 Fix: Actually save training history files
    if pipeline.training_history["episode_rewards"]:
        ...
        try:
            ...
            # 2. Verify if file was actually created
            ...
            try:
                # Save entire training_history, not just episode_rewards
        ...
        logger.warning("⚠️ pipeline.training_history['episode_rewards'] is empty, training failed or not trained")
