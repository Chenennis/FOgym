...
class ManagerAgent:
    """Manager agent class, managing a group of users and devices"""
    ...
    def _initialize_devices(self):
        ...
        logger.info(f"Manager {self.manager_id}: Initialized {len(self.device_mdps)} devices, "
                   f"of which {len(self.controllable_devices)} are controllable")
    ...
    def get_state_features(self, standardized: bool = True) -> np.ndarray:
        """
        Get Manager's state features - optimized version
        
        Args:
            standardized: Whether to apply standardized processing
            
        Returns:
            np.ndarray: Standardized Manager state feature vector
        """
        if standardized:
    ...
    def _get_standardized_state_features(self) -> np.ndarray:
        ...
        # 1. Device feature module (organized and standardized by type)
        device_features = self._get_standardized_device_features()
        
        # 2. Manager management feature module (standardized)
        management_features = self._get_standardized_management_features()
        
        # 3. User preference feature module (standardized)
        preference_features = self._get_standardized_preference_features()
        
        # 4. System state feature module (standardized)
        system_features = self._get_standardized_system_features()
        
        # Merge all feature modules
        all_features = np.concatenate([
            device_features,      # Device state (the most important feature)
            management_features,  # Management feature
            preference_features,  # User preference
    ...
    def _get_standardized_device_features(self) -> np.ndarray:
        ...
        # Collect enhanced features for each type of device
        ...
        # Generate aggregated features instead of fixed dimension
        aggregated_features = []
        
        for device_type in sorted(device_aggregation.keys()):
            features_list = device_aggregation[device_type]
            if features_list:
                # Aggregate features of the same type of device: [number, average, maximum, minimum, standard deviation]
                stacked = np.stack(features_list)
                aggregated = np.array([
                    float(len(features_list)),   # Number of devices
                    float(np.mean(stacked)),     # Average
                    float(np.max(stacked)),      # Maximum 
                    float(np.min(stacked)),      # Minimum
                    float(np.std(stacked))       # Standard deviation
                ])
            else:
        ...
        # Fixed 25-dimensional device aggregation features (5 types × 5 dimensions/type)
    ...
    def _get_enhanced_device_features(self, device_id: str, device_type: str, mdp) -> np.ndarray:
        ...
        if device_type == DeviceType.BATTERY:
            ...
            hour = 12  # Simplified to a fixed time
            generation_potential = 0.8 if 8 <= hour <= 16 else 0.2
            return np.array([generation_potential])
            
        elif device_type == DeviceType.DISHWASHER:
            # Simplified dishwasher features: [running state, progress]
    ...
    def _get_standardized_management_features(self) -> np.ndarray:
        ...
        # Region type encoding (one-hot encoding)
    ...
    def _get_legacy_state_features(self) -> np.ndarray:
        """Get traditional state features (keeping backward compatibility)"""
    ...
    def get_action_space_size(self) -> int:
        """
        Each controllable device's action contains:
        ...
        """
        # Each controllable device needs 5D FlexOffer parameters
        fo_params_per_device = 5
        total_action_dim = len(self.controllable_devices) * fo_params_per_device
        
        # Ensure at least basic action dimensions
    ...
    def step(self, actions: np.ndarray, env_state: Dict) -> Tuple[float, Dict]:
        """
        Execute one action - modified to complete Pipeline process
        ...
        # Step 4: Calculate Pipeline execution reward
    ...
    def _generate_device_flexoffers(self, fo_params: Dict, env_state: Dict) -> Dict:
        ...
        for device_id, params in fo_params.items():
            if device_id in self.device_mdps:
                ...
                # Add time slice (using correct DFOSlice constructor parameters)
                dfo_slice = DFOSlice(
                    time_step=0,
    ...
    def _aggregate_flexoffers(self, device_flexoffers: Dict, env_state: Dict) -> List:
        ...
        try:
            ...
            logger.info(f"Number of device FlexOffers: {len(flex_offers)}")
            
            # Ensure aggregation method is uppercase to match logic in FOAggregatorFactory
            ...
            if flex_offers:
                ...
                # Add aggregation result logging
                if aggregated_result:
                    logger.info(f"Aggregation successful: got {len(aggregated_result)} aggregated FlexOffers")
                    for i, afo in enumerate(aggregated_result):
                        logger.info(f"Aggregation result #{i+1}: method={afo.aggregation_method}, "
                                   f"source FO count={len(afo.source_fo_ids)}, "
                                   f"total energy range=[{afo.total_energy_min:.2f}, {afo.total_energy_max:.2f}]")
                else:
                    logger.warning("Aggregation completed, but no aggregated results")
    ...
    def _trade_flexoffers(self, aggregated_results: List, env_state: Dict) -> List:
        """Trade FlexOffer - call fo_trading module's real algorithm"""
        try:
    ...
    def _trade_with_bidding(self, aggregated_results: List, env_state: Dict) -> List:
        """Use Bidding algorithm to trade - generate trade when both prices match"""
        try:
            ...
            for i, aggregated_fo in enumerate(aggregated_results):
                # Fix: correctly get energy from aggregated FlexOffer
                total_energy = 0.0
                ...
                # Ensure total energy is at least 1.0 to avoid zero energy transactions
                ...
                # Create sell bid (current Manager) - lower sell price to increase transaction probability
                sell_price = base_price * (0.85 + 0.15 * np.random.random())  # 0.85-1.0x base price, lower sell price
                sell_bid = Bid(
                ...
                # Create simulated buy bid - higher buy price to increase transaction probability
                buy_price = base_price * (1.05 + 0.25 * np.random.random())  # 1.05-1.3x base price, higher buy price
                buy_quantity = total_energy * (0.8 + 0.4 * np.random.random())  # 0.8-1.2x energy demand
                buy_bid = Bid(
            ...
            # Bidding algorithm: check if buy and sell prices match, if match then generate trade
            trades = []
            ...
            # Match buy and sell bids: trade when buy price >= sell price
            for sell_bid in sell_bids:
                if sell_bid.participant_id != self.manager_id:
                    continue  # Only process current Manager's sell bids
                    
                for buy_bid in buy_bids:
                    if buy_bid.price >= sell_bid.price:  # Price match
                        # Calculate trade quantity (take minimum of both)
                        trade_quantity = min(sell_bid.quantity, buy_bid.quantity)
                        
                        # Calculate trade price (average of buy and sell prices)
                        ...
                        # Update bid quantity (simplified, should be more complex)
                        sell_bid.quantity -= trade_quantity
                        buy_bid.quantity -= trade_quantity
                        
                        logger.info(f"Trade successful (Bidding): buyer {buy_bid.participant_id}({buy_bid.price:.4f}) "
                                  f"vs seller {sell_bid.participant_id}({sell_bid.price:.4f}), "
                                  f"trade price {trade_price:.4f}, quantity {trade_quantity:.2f}")
                        
                        # If buyer demand is satisfied, jump to next buyer
                        if buy_bid.quantity <= 0:
                            break
                    
                    # If seller supply is used up, jump to next seller
    ...
    def _trade_with_market_clearing(self, aggregated_results: List, env_state: Dict) -> List:
        """Use Market Clearing algorithm to trade"""
        try:
            ...
            for i, aggregated_fo in enumerate(aggregated_results):
                ...
                # Create sell bid (current Manager) - keep consistent price range with bidding algorithm
                sell_bid = Bid(
                    bid_id=f"sell_{self.manager_id}_{i}",
                    participant_id=self.manager_id,
                    price=base_price * (0.85 + 0.15 * np.random.random()),  # 0.85-1.0x base price
                    quantity=total_energy,
                    time_step=0,
                    side="sell"
                )
                
                # Create multiple simulated buy bids (increase transaction probability) - expand price range
                for j in range(2):  # Create 2 buy bids for each FO
                    buy_bid = Bid(
            ...
            for trade in generated_trades:
                if trade.seller_id == self.manager_id:  # Only care about current Manager's trades
                    trades.append({
                ...
                if trade.get('success', False):
                    ...
                    if original_data:
                        ...
                        if total_energy <= 0:
                            ...
                            # Directly allocate zero energy to all devices, avoid disaggregation algorithm error
    ...
    def _calculate_device_satisfaction(self, device_mdp, power_signal: float, 
        ...
        try:
            ...
            return 0.5  # Default medium satisfaction
    ...
    def _calculate_pipeline_reward(self, pipeline_results: Dict, env_state: Dict) -> Tuple[float, Dict]:
        ...
        if net_benefit > 20:
            economic_reward = 70.0 + min(net_benefit - 20, 30.0)  
        elif net_benefit > 5:
            economic_reward = 40.0 + (net_benefit - 5) * 2.0  
        elif net_benefit > 0:
            economic_reward = 20.0 + net_benefit * 4.0  
        elif net_benefit > -5:
            economic_reward = 10.0 + (net_benefit + 5) * 2.0  
        else:
            economic_reward = max(0.0, 10.0 + net_benefit)  
        
        satisfaction_base = stats.get('avg_satisfaction', 0.5)
        
        if satisfaction_base > 0.8:
            satisfaction_reward = 60.0 + (satisfaction_base - 0.8) * 100  
        elif satisfaction_base > 0.6:
            satisfaction_reward = 40.0 + (satisfaction_base - 0.6) * 100  
        elif satisfaction_base > 0.4:
            satisfaction_reward = 20.0 + (satisfaction_base - 0.4) * 100  
        elif satisfaction_base > 0.2:
            satisfaction_reward = 10.0 + (satisfaction_base - 0.2) * 50  
        else:
            satisfaction_reward = max(10.0, satisfaction_base * 50)  
        ...
        if trade_success_rate > 0.7:
            coordination_reward = 40.0 + (trade_success_rate - 0.7) * 66.7  
        elif trade_success_rate > 0.4:
            coordination_reward = 20.0 + (trade_success_rate - 0.4) * 66.7  
        elif trade_success_rate > 0.1:
            coordination_reward = 5.0 + (trade_success_rate - 0.1) * 50  
        else:
            coordination_reward = max(5.0, trade_success_rate * 50)  
        
        strategy_consistency_reward = 15.0  
        if hasattr(self, 'markov_history') and 'prev_actions' in self.markov_history:
            ...
            if prev_actions is not None and len(prev_actions) > 0:
                ...
                # Reward reasonable strategy
                if 0.2 <= action_std <= 0.6 and 0.3 <= action_mean <= 0.7:
                    strategy_consistency_reward = 30.0  
                elif 0.1 <= action_std <= 0.8 and 0.2 <= action_mean <= 0.8:
                    strategy_consistency_reward = 20.0  
                elif action_std < 0.05:  # Strategy too conservative
                    strategy_consistency_reward = 5.0
                elif action_std > 0.9:  # Strategy too random
                    strategy_consistency_reward = 8.0
                else:
                    strategy_consistency_reward = 15.0  
        

        # Target range: 30-270 points
        total_reward = (
            0.4 * economic_reward +           
            0.3 * satisfaction_reward +       
            0.2 * coordination_reward +       
            0.1 * strategy_consistency_reward 
        )  
        ...
        if self.episode_count <= 20:
            ...
            learning_progress_reward = np.random.uniform(-5, 15)  
        ...
        total_reward = max(30.0, min(200.0, total_reward))  
    ...
    def get_observation(self):
        ...
        # Ensure observation dimension consistency (here no forced constraint, because observation dimension is dynamic)
...
class MultiAgentFlexOfferEnv(gym.Env):
    ...
    def __init__(self, 
        ...
        # Dynamic observation quality manager (if observation noise is enabled, enable dynamic quality)
    ...
    def set_algorithms(self, aggregation: Optional[str] = None, trading: Optional[str] = None, disaggregation: Optional[str] = None):
        """Dynamic set algorithm selection"""
        if aggregation:
    ...
    def _create_manager_agents(self):
        ...
        for _, manager_row in self.manager_config_df.iterrows():
            ...
            for user_id in user_ids:
                if user_id.startswith('user_'):
                    try:
                        ...
                        # Calculate the local number of the user in the Manager
            ...
            # Match devices using the extended user ID list
    ...
    def _setup_spaces(self):
        ...
        for manager_id, manager in self.manager_agents.items():
            # Get the state feature dimension of a single Manager
            ...
            # Basic information 5D + enhanced information 9D = 14D for each Manager
        ...
        for manager_id in self.manager_ids:
            ...
            if total_action_dim > 0:
                ...
                # If there are no controllable devices, create a virtual action space
                self.action_spaces[manager_id] = spaces.Box(
                    low=-1.0, high=1.0, shape=(5,), dtype=np.float32
                )
                logger.warning(f"Manager {manager_id} has no controllable devices, using virtual action space")
        
        logger.info("Observation and action space setup completed - FlexOffer parameter generation mode")
    ...
    def step(self, actions: Dict[str, np.ndarray]):
        ...
        # Add trading algorithm configuration to environment state
    ...
    def _get_observations(self) -> Dict[str, np.ndarray]:
        """
        Get observations of all Managers - enhanced Dec-POMDP version
        Implement strict hierarchical observation space: O_i = [O_private_i, O_public, O_limited_others_i]

        """
        observations = {}
        
        # Ensure that all Managers get the same public environment features (no noise) at the same time step
        ...
        for manager_id, manager in self.manager_agents.items():
            ...
            # public_features has already been obtained above
            ...
            # Others information: Apply information transmission mechanism (possibly noisy and delayed)
            if len(limited_others_features) > 0:
                ...
                # Merge three layers of observations: private + public (no noise) + processed others
                arrays_to_concat = []
    ...
    def _get_standardized_time_features(self) -> np.ndarray:
        """
        Get standardized time feature layer
        
        Time information encoding principles:
        1. Periodic time encoding: sin/cos to avoid boundary problems
        2. Multi-scale time representation: hour, day, week, season
        3. Progress indicator: task completion and remaining time
        
        Returns:
            np.ndarray: 6D time feature vector
        """
    ...
    def _get_standardized_market_features(self) -> np.ndarray:
        ...
        # Standardize current price (relative to base price)
    ...
    def _validate_public_information_layer(self, public_features: np.ndarray, 
        ...
        # Time layer validation: sin/cos value should be in [-1,1], others should be in [0,1]
        ...
        assert abs(time_layer[0] - expected_hour_sin) < 0.01, f"Hour encoding inconsistency: {time_layer[0]} vs {expected_hour_sin}"
    ...
    def _calculate_dynamic_aggregation_weights(self) -> Dict[str, float]:
        ...
        weights = {
            'scale_weight': max(0.3, 1.0 - time_progress),  # Early weight high
            'performance_weight': min(1.0, 0.5 + time_progress),  # Late weight high
            'collaboration_weight': 0.6 + 0.2 * math.sin(time_progress * math.pi),  # Mid weight high
    ...
    def _aggregate_performance_metrics(self, manager: 'ManagerAgent', 
        ...
        # Energy efficiency indicator (energy consumption relative to scale)
    ...
    def _apply_adaptive_aggregation_strategy(self, manager: 'ManagerAgent', 
        ...
        coordination_need = 1.0 - system_stats['system_balance']  # The more unbalanced the system, the higher the coordination need
        
        # 3. Adaptive response ability
    ...
    def _apply_multi_level_delay(self, observation: np.ndarray, manager_id: str) -> np.ndarray:
        ...
        if system_load > 0.7:  # When system load is high, increase delay
    ...
    def _apply_transmission_quality_degradation(self, observation: np.ndarray, manager_id: str) -> np.ndarray:
        ...
        distance_factor = 1.0  
        time_factor = 1.0 - (self.current_step / self.time_horizon) * 0.05  
    ...
    def _apply_information_recovery_mechanism(self, observation: np.ndarray, manager_id: str) -> np.ndarray:
        ...
        valid_indices = np.where(np.abs(recovered_observation) > 1e-8)[0]  
        invalid_indices = np.where(np.abs(recovered_observation) <= 1e-8)[0]  
                ...
                if idx < len(last_valid_observation):
                    if len(self.recovery_cache[manager_id]) >= 2:
                        ...
                        if len(recent_values) >= 2:
                            ...
                            estimated_value = recent_values[-1] + trend * 0.5  
    ...
    def _get_legacy_detailed_manager_info(self) -> Dict[str, List[float]]:
        ...
        for manager_id, manager in self.manager_agents.items():
            legacy_info = [
                len(manager.users),  
                len(manager.device_mdps),  
                manager.markov_history['cumulative_cost'],  
                manager.markov_history['cumulative_energy'],  
                manager.markov_history['user_satisfaction'],  
    ...
    def _get_market_state_features(self) -> np.ndarray:
        ...
        recent_activity = min(1.0, self.current_step / 5.0)  
        ...
        market_features = np.array([
            ...
            1.0 if satisfaction_mean > 0.5 else 0.0,  
            1.0 if energy_balance < 0.5 else 0.0,     
        ])
    ...
    def generate_current_dfos(self, timestep):
        ...
        for manager_id, agent in self.manager_agents.items():
            ...
            for device in devices_list:
                ...
                energy_min = np.random.uniform(5, 20)     
                energy_max = np.random.uniform(20, 50)    
                power_min = np.random.uniform(-10, 0)     
                power_max = np.random.uniform(5, 15)      
        ...
        logger.info(f"Time step {timestep} DFO generation: {', '.join(manager_dfo_counts)}, total {total_dfos}")
        return dfo_systems
    
    def update_user_states(self, user_satisfied_energy, timestep):
        """
        Update user status based on allocated energy
        """
        try:
            for manager_id, agent in self.manager_agents.items():
                for user in agent.users:
                    # Process user objects: users may be a list of dictionaries
                    if isinstance(user, dict):
                        user_id = user.get('user_id', '')
                        user_devices = user.get('devices', [])
                    else:
                        # If it is an object, access the attribute directly
                        user_id = getattr(user, 'user_id', '')
                        user_devices = getattr(user, 'devices', [])
                    
                    if user_id:
                        try:
                            # Parse user ID format: supports user_X and user_manager_X_Y formats
                            if 'manager_' in user_id:
                                ...
                                if len(parts) >= 4:
                                    ...
                                    user_local_num = int(parts[3])  # manager user number (1, 2, ...)
                            ...
                            if user_idx < user_satisfied_energy.shape[0]:
                                ...
                                if user_devices and satisfied_energy > 0:
                                    ...
                                    for device in user_devices:
                                        # Process device objects: devices may be a dictionary
                                        ...
                                        if device_id:
                                            ...
                                            if device_key in agent.device_mdps:
                                                ...
                                                if hasattr(mdp_device, 'env'):
                                                    # Update device state to reflect the energy obtained
    ...
    def _update_device_state_with_energy(self, device_env, allocated_energy):
        ...
        try:
            ...
            if hasattr(device_env, 'device_type'):
                ...
                # Additional device type state updates can be added here
        except Exception as e:
